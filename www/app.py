...
async def data_factory(app, handler):
    async def parse_data(request):
        if request.method == 'POST':
            ...
            if request.content_type.startswith('application/x-www'):
                request.__data__ = await request.post()
                logging.info('request post: %s' % str(request.__data__))
        return (await handler(request))
    return parse_data


async def auth_fsctory(app, handler):
...
async def init(loop):
    ...
    app = web.Application(loop=loop, middlewares=[
        logger_factory, auth_fsctory, response_factory
    ])
